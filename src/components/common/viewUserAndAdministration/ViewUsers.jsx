...
import { useGetUsersQuery } from "../../../redux/slices/authSlice";
import EditUserModal from "./modal/EditUserModal";
import DeleteUserModal from "./modal/DeleteUserModal";

const TABLE_HEAD = ["S.No", "Name", "Email", "Mobile Number", "Designation", "Date", "Edit", "Delete"];
...
export default function ViewUser() {
    ...
    const handlePrevious = () => {
        const totalPages = Math.ceil((users?.pagination?.total ?? 0) / limit);
    ...
    const handleDownload = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_HOST_URL}/api/v1/user/users/download`, {
                method: 'GET',
        ...
        <div className="h-full w-full bg-white pt-1 rounded-md border border-purple-300">
            ...
            <div className="overflow-scroll p-2">
                {isLoading ? (
                    ...
                    <table className="w-full min-w-max table-auto text-left">
                        ...
                        <tbody>
                            {users?.data?.map(
                                ({ _id, name, email, mobileNumber, role, createdAt }, index) => (
                                    <tr key={index} className="hover:bg-purple-50/50 cursor-pointer">
                                        ...
                                        <td className="px-5  border-l border-r border-b border-purple-300">
                                            <EditUserModal id={_id} refetch={refetch} role={role} />
                                        </td>
                                        <td className="px-5  border-l border-r border-b border-purple-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {users?.data?.map(({ _id, name, email, mobileNumber, role, createdAt }) => (
            ...
            <div className="flex items-center justify-between border-t border-purple-300 p-4">
                <Typography variant="small" color="blue-gray" className="font-normal">
                    Page {page} of {Math.ceil((users?.pagination?.total ?? 0) / limit)}
                </Typography>
                <div className="flex gap-2">
                    ...
                    <Button
                        ...
                        onClick={handleNext}
                        disabled={page === Math.ceil((users?.pagination?.total ?? 0) / limit)}
                    >
                        Next
                    </Button>
                </div>
